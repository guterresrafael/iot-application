...
public class JettisonMappedMarshaller extends BaseMarshaller
{
           ...
           throws JAXBException
   {
       ...
       Reflection.getAllFields(fields, clazz);
       for (Field field : fields) {
           field.setAccessible(true);
           try {
               if (field.get(o) instanceof Collection<?>) {
                   mapped.serializeAsArray(field.getName());
               }
           } catch (IllegalArgumentException ex) {
               Logger.getLogger(JettisonMappedMarshaller.class.getName()).log(Level.SEVERE, null, ex);
